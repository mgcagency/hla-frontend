...
import { addWellness, getWellness } from "../../../api/Admin/wellness";
...
const Wellness = () => {
  ...
  const fetchData = async() =>{
    setLoading(true);
    const resp = await getWellness();
    if(resp.success){  
      setData(resp?.data);
      console.log("wellness data is : ", resp);
    }
    setLoading(false);
  }

  const handleCreateWellness = async () =>{
    const resp = await addWellness({formLink});
  }
            ...
            <div>
              ...
              {!loading &&
                (currentStudents.length === 0 ? (
                  ...
                  <div>
                    {currentStudents[0].submissions.map((student) => {

...
export default function ParentDeletedPopup({ parent, toggleFunc }) {
  const { refetch } = useGetUsers();
  const [loader, setLoader] = useState(false);

  const handleDeleteClick = async () => {
    setLoader(true);
    console.log("delete function called: ", parent?._id);
    try {
            ...
            <span className="name font-semibold text-customDarkBlue">
              <span className="items-center">
                <img
                  src={parent?.photo}
                  alt="avatar"
                  className="w-6 h-6 mr-1 inline-block rounded-full"
                />
                {parent?.name}
              </span>
            </span>{" "}
